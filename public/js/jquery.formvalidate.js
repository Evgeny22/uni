...
(function($) {
	...
	$.fn.formvalidate.options = {
			...
			$(form).find(':input').each(function(index, element) {
				...
				$.each(options.validations, function(validation, method) {
					...
					// If this is the required_if validation
						...
						if ('requiredIfNot' in data) {
							inputs[attrName].validations.requiredIfNot = [ data.requiredIfNot ];
						}
					}
					...
					$.each(inputObj.validations, function(validationName, validationParams) {
						...
						if (inputObj.failure === null) {
							...
							if (validationName in options.validations) {
								...
								if (inputObj.value === null && validationName !== 'required' && validationName !== 'requiredIf'  && validationName !== 'requiredIfNot') {
		...
		_displayMessages: function(form, inputs) {
			...
			// Scroll to errors class
            if (form.options.failureMessages === true) {
            	/*console.log('errors! class; '+ form.options.inputFailureClass);
                $('html, body').animate({
                    scrollTop: $("."+ form.options.inputFailureClass).offset().top
                }, 2500);*/
			}
		},
		/**
		...
		messageElement: '<div />', // Wrap success and failure messages inside this element
		...
		localization: {
			en: {
				failure: {
					...
					numOptions: 'Must select exactly {2} category checkbox below.',
					minOptions: 'Must select at least {2} category checkbox below.',
					maxOptions: 'Cannot select more than {2} category checkbox below.',
					'int': '{0} must be a whole number (integer).',
					'float': '{0} must be a valid number.',
					required: '{0} is required.',
					requiredIf: '{0} is required.',
					requiredIfNot: '{0} is required.',
		...
		validations: {
			...
			requiredIfNot: function(input, params) {
				// Dependent element
				var el = $('#' + params[0]);
